...
class SnakeGame extends StatelessWidget {
  const SnakeGame({super.key, this.onExitRequested});
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<GameState>(
      create: (context) => GameState(onExitRequested: onExitRequested),
...
class SnakeHomeState extends State<SnakeHome>
  ...
  Widget _buildMainPanel(BuildContext context, double width, double height) {
    ...
    if (gameState.initFlag) {
      blockNum = gameState.initSize;
    } else {
      blockNum = gameState.mapSize;
    }
    double brickSize = size / blockNum;
    return InkWell(
      onTap: () {
  ...
  Container _circularButton(double size, IconData icon, Color bkColor,
    ...
    return Container(
      ...
      child: IconButton(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: bkColor),
        onPressed: () {
