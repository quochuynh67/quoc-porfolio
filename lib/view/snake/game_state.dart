...
class GameState with ChangeNotifier {
  GameState({this.onExitRequested}) {
    _initSettings();
    if (_launchIconFlag) {
      _initSize = 21;
      _generateLaunchIcon();
  ...
  void resetColorPlan(BuildContext context) async {
    ...
    if (saveOptions[confirm] == 1) {
      var jsonString = await rootBundle.loadString(
          'assets/images/projects/snake/cfg/default.json');
  ...
  Future<void> _buttonFeedback() async {
    ...
    try {
      ...
      // Haptics are optional for web/desktop embedding.
    }
  }
  ...
  Future<void> menuExit(BuildContext context) async {
    _playFlag = false;
    _timer.cancel();
  ...
  Future<void> _persistPreferences() async {
    ...
    await prefs.setInt('appBk', _appBkColor);
    await prefs.setInt('brickBk', _brickBkColor);
    await prefs.setInt('leftPlayer', _leftPlayerColor);
    await prefs.setInt('rightPlayer', _rightPlayerColor);
    await prefs.setInt('controllerShadow', _controllerShadowColor);
    await prefs.setInt('brickOnePlayer', _brickOnePlayerColor);
    await prefs.setInt('scoreBoard', _scoreBoardColor);
    await prefs.setInt('food', _foodColor);
