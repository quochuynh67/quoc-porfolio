...
import '../global/components/barrier.dart';
import '../global/components/distance.dart';
import '../global/components/game_level.dart';
import '../global/model/actor.dart';
import '../presentation/widgets/controls.dart';
import '../presentation/widgets/dialog_manager.dart';
...
//<----------------------------------Actors------------------------------------>
  ...
  Thorn _movingThorn = Thorn(dx: 7.0, url: 'assets/images/projects/bounce/moving_thorn.png');
  ...
  void jump() {
    if (_time != 0) {
      ...
      Timer.periodic(_refreshDuration, (timer) {
        ...
        final nextBallDy = _initialHeight - _height;

        //Check for barrier
        final List<Barrier> _barriers = _level.barrierList;
        bool _handledCollisionZone = false;
...
//<----------------------------------Helpers----------------------------------->
  ...
  void _checkIfRingCollected() {
    for (Actor ring in _ringsList) {
      if (ring.isCollected == false &&
          _position.dx > (ring.impactDx ?? 0.0) &&
          _position.dx < (ring.impactDx ?? 0.0) + 0.1 &&
          _ball.dy >= ring.dy) {
        if (ring.dy.isNegative) {
        } else {
          smallJump();
        }
        ring.url = 'assets/images/projects/bounce/ring_collected.png';
...
//<--------------------------- Collision Zone --------------------------------->
      if (_checkColission) {
        ...
        if (_barrier.object == Thorn && _ball.dy >= _barrier.dy) {
          _ball.url = 'assets/images/projects/bounce/ball_pop.png';
